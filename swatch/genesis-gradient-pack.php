<?php
     $og_image = 'https://mjmacfadden.github.io/cryptoswatches/img/featured-img/genesis-gradient-pack-featured-img.jpg'; // Set before including header
    include '../header.php'; // Include header
?>
<div class="swatch-hero position-relative" style="background-color:;">
  <div class="ratio ratio-16x9">
    <video autoplay muted loop playsinline poster="poster.jpg">
      <source src="vid/genesis-gradient-pack-opening.mp4" type="video/mp4">
      Your browser does not support HTML5 video.
    </video>
  </div>

  <!-- Chevron overlay -->
  <a href="swatch/genesis-gradient-pack.php#swatch-description" 
     class="position-absolute bottom-0 start-50 translate-middle-x mb-3 text-white fs-1">
    <i class="bi bi-chevron-double-down"></i>
  </a>
</div>

<div class="container my-4 shadow-img">
    <div class="row mt-4 pb-5" id="swatch-description">
        <h1>The Genesis Gradient Pack</h1>
        <h4>by Michael Macfadden</h4>
        <h2>Common Crypto Swatches</h2>
        <p>The Genesis Gradient pack includes ten crypto swatches. Upon opening, nine of your ten crypto swatches will be of a common rarity. For each of those nine, you have an equal chance of receiving a yellow, green, or red crypto swatch.</p>
    </div>
    <div class="row">
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000080.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000081.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000082.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
    </div>
    <div class="row mt-4 pb-5" id="swatch-description">
        <h2>Dual-Blend Gradients</h2>
        <p>The 10th card offers an equal chance of receiving one of three dual gradients: a yellow/green, a yellow/red, or a red/green. Don&rsquo;t worry though. You can&nbsp;<a title="Blend" href="https://neftyblocks.com/c/cryptoswatch/blends" target="_blank" rel="noopener">blend</a>&nbsp;your common crypto swatches to make a dual gradient.</p>
    </div>
    <div class="row">
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000083.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000084.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
        <div class="col d-flex justify-content-center align-items-center mb-4">
            <img src="img/000085.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
    </div>
</div>
<div style="background:#f1f1f1;" class="py-5">
    <div class="container my-4 shadow-img">
        <div class="row mt-4 pb-5" id="swatch-description">
            <div class="col-sm-8">
                <h2>Tri-Blend Gradients</h2>
                <h3>Max Supply 100</h3>
                <p>Only 100 Tri-Blend Gradient Crypto Swatches will ever exist. Earn one by blending each of the Dual-Blend Gradients above.</p>
                <p>Lucky pack owners could obtain a tri-blend by purchasing just one pack.</p>
                <p>In many cases, you can obtain the entire set by purchasing just two packs. Best of luck!</p>
            </div>
            <div class="col-sm-4 d-flex justify-content-center align-items-center mb-4">
                <img src="img/000086.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
        </div>
    </div>
</div>

<div class="swatch-footer py-5" style="background-color:#161616;">
    <div class="container my-4 shadow-img">
        <div class="row mt-4">
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000080.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000081.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000082.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000083.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000084.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
            <div class="col d-flex justify-content-center align-items-center mb-4">
                <img src="img/000085.png" class="img-fluid lightbox" alt="Genesis Gradient Pack">
            </div>
        </div>
        <div class="row">
            <div class="col-6">
                <a href="swatch/konceptonwax.php"><i class="bi bi-arrow-left"></i> KONCEPTONWAX</a>
            </div>
            <div class="col-6 text-end">
                <a href="swatch/victorysher.php">VICTORY SHER <i class="bi bi-arrow-right"></i></a>
            </div>
        </div>
    </div>
</div>


<?php
    include '../footer.php'; // Include footer
?>
